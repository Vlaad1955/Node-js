...
class AuthMiddleware {
    ...
    public async checkNewPasswordNotInHistory(
        req: Request,
        res: Response,
        next: NextFunction,
    ): Promise<void> {
        try {
            ...
            const isInHistory = await passwordHistoryRepository.isPasswordInHistory(userId, password);
            if (isInHistory) {
                throw new ApiError("The new password cannot be the same as an old password.", 400);
            }
